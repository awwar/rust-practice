...
pub fn get_procedures() -> Vec<Box<dyn Procedure>> {
    vec![Box::new(FillRandom::new()), Box::new(At {})]
}
...
impl Procedure for FillRandom {
    ...
    fn execute(&self, argc: usize, stack: &mut Stack) {
        ...
        let Value::Integer(max) = stack.pop() else {
            todo!()
        };
        let Value::Integer(min) = stack.pop() else {
            todo!()
        };
        let Value::Integer(size) = stack.pop() else {
            todo!()
        };
        let Value::Array(array) = stack.pop() else {
            todo!()
        };

        let mut new_val = array.clone();
        let addition = (0..size)
            .map(|_| Value::Integer(self.rng.borrow_mut().random_range(min..max)))
            .collect::<Vec<Value>>();
...
impl Procedure for At {
    ...
    fn execute(&self, argc: usize, stack: &mut Stack) {
        ...
        let Value::Integer(count) = stack.pop() else {
            todo!()
        };
        let Value::Array(array) = stack.pop() else {
            todo!()
